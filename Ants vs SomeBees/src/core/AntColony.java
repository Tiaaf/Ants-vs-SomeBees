...
public class AntColony {
	...
	public AntColony (int numTunnels, int tunnelLength, int moatFrequency, int startingFood) {
		...
		for (int tunnel = 0; tunnel < numTunnels; tunnel++) {
			...
			for (int step = 0; step < tunnelLength; step++) {
				...
				if (moatFrequency != 0 && moatCounter%moatFrequency == 0){
					curr = new Water ("tunnel[" + tunnel + "-" + step + "]", prev);//creates a new moat
				}else{
	...
	/**
	 * Sets the queen's location
	 * 
	 * @param queenPlace
	 * 				the current queen's location
	 * @author Adam & Antonin
	 */
	public void setQueenPlace (QueenPlace queenPlace) {
	...
	public void deployAnt (Place place, Ant ant) {
		if (food >= ant.getFoodCost()&& place.addInsect(ant)){
	...
	public ArrayList<Ant> getAllAnts () {
		...
		for (Place p : places) {
			if (p.getAnt() != null) {
				ants.add(p.getAnt());
				if (p.getAnt() instanceof Containing && ((Containing) p.getAnt()).getContainedAnt() != null){
					ants.add(((Containing) p.getAnt()).getContainedAnt());
				}
			}
		}
		return ants;
